...
; Begin at x3000 and clear registers for storage
                    .orig x3000
...
; Display (E)ncrypt / (D)ecrypt message and store the result
                    ...
                    OUT
                    LEA     R0, NEWLINE           ; Display a new line
                    PUTS
...
; continue until we receive the return key as a character
...
END                 AND R0, R0, #0              ; R0 <- NULL terminating character
...
ENCRYPT             JSR     Encrypt_SR          ; Jumps to the Encrypt subroutine
...
Encrypt_SR
                    ...
                    LD      R1, INPUT_MESSAGE   ; Load the memory address of the first char in the given string
                    LDI     R4, INPUT_KEY       ; Let R4 be an integer between (1-9) for the char shift position.
...
ENCRYPT_EVEN        ADD     R3, R3, 1           ; else bit[0:0] is 0 (even)
...
DONE_ENCRYPT        AND     R0, R0, #0          ; R0 <- NULL terminating character
...
DONE_DECRYPT        AND     R0, R0, #0          ; R0 <- NULL terminating character
                    STR     R0, R6, #0          ; Store the null terminating character at the end of the string.                  
                    LD      R7, BACKUP_R7       ; Load our return address into R7
                    RET                         ; JMP R7
...
PROMPT_KEY              .STRINGZ "Enter encryption key(1-9): "
...
INPUT_MESSAGE   .FILL   x3102   ; This needs to take up 20 registers + 1 for the termination
BACKUP_R7       .BLKW   1       ; Reserver one block of memory for when we need to save R7
RESULT          .FILL   x3117   ; (x3102 + 21 in base 10) gives us the next available space to save a value
